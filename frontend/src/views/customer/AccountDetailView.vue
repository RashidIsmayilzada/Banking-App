<template>
  <CustomerShell>
    ...
    <div v-if="error" class="card" style="padding:20px;color:red">{{ error }}</div>
    <div v-if="loading" class="card" style="padding:32px;text-align:center">Loading account...</div>

    <template v-if="account">
      <div class="row" style="margin-bottom:28px;gap:16px">
        <span class="badge badge--dark">{{ account.accountType === 'CHECKING' ? 'Checking' : 'Savings' }}</span>
        <AppStatus :kind="account.status === 'ACTIVE' ? 'active' : 'closed'" />
      </div>
      <div class="t-display" style="margin:0;font-size:64px">
        {{ formatEur(account.balance.amount) }}
      </div>
      <div class="iban" style="margin-top:8px;font-size:14px">{{ account.iban }} · BIC INHONL2A</div>
        ...
        <div class="col" style="gap:16px">
          <div class="card">
            ...
            <div class="row" style="justify-content:space-between">
              ...
              <span style="color:var(--teal);font-weight:500">—</span>
            </div>
...
const accountDetails = computed(() => {
  ...
  return [
    ['Account type',                a.accountType === 'CHECKING' ? 'Checking' : 'Savings', false],
    ['Daily transfer limit',        formatEur(a.dailyTransferLimit.amount),                 false],
    ['Absolute limit (min balance)', formatEur(a.absoluteTransferLimit.amount),              false],
    ['Opened on',                   formatDate(a.createdAt),                                false],
    ['BIC',                         'INHONL2A',                                             true ],
  ]
})
</script>
